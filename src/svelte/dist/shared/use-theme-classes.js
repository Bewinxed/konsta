...
const themeClasses = (classesObj, theme, addBaseClassName) => {
    ...
    Object.keys(classesObj).forEach((key) => {
        ...
        const hasStates = typeof classesObj[key] !== 'string' &&
            Object.keys(classesObj[key]).filter((stateKey) => !themeSubKeys.includes(stateKey)).length > 0;
        if (!hasStates) {
        ...
        Object.keys(classesObj[key])
            ...
            .forEach((stateKey) => {
            c[key][stateKey] = cls(defaultStateClasses, propClasses(classesObj[key], theme, stateKey), addBaseClass);
        });
    });
    return c;
};
...
const useThemeClasses = (props, classesObj, cb, addBaseClassName = '') => {
    ...
    const calcClasses = () => {
        return themeClasses(typeof classesObj === 'function' ? classesObj() : classesObj, theme, addBaseClassName);
    };
    if (props.ios)
        theme = 'ios';
